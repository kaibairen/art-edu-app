...
| 2a | F-011 / P0.1 | 管理端+教师 | 教师 `classNames: string[]` 管理端可编辑；教师空态「请联系管理员分配班级」 | 正式类型 `@art-edu/api-types`：`Account.displayName` / `status` / `classNames[]`；创建/更新用 `CreateAccountRequest` / `UpdateAccountRequest`（无 `name`/`disabled`）。停用走 `PATCH /admin/accounts/{id}/status`。教师 Home 空列表文案见上。 [docs/P0.1-教师班级归属小补.md](docs/P0.1-教师班级归属小补.md) |
